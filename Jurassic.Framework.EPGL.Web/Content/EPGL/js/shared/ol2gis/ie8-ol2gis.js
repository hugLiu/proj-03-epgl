...
*/
    ...
    Ogis.prototype.initMap = function () {
        ...
        */
        var map = new OpenLayers.Map("gis", {  
            maxExtent: model.extent, //地图表示最大范围（左, 下, 右, 上） ，可以作为默认值  
            numZoomLevels: model.maxZoom,  //地图缩放的最大级别（根据自己的实际需求设置）  
            //maxResolution : 156543.0339,   //地图缩最大像素，（默认）  
            units:'m', //地图单位米（默认）  
            projection: model.projection,//采用投影坐标系（默认）
            displayProjection: model.projection//采用经纬度坐标系（默认） 
        });
    ...
    Ogis.prototype.initMapBaseLayer = function () {
        ...
        var layer = new OpenLayers.Layer.GoogleLayer("basemap",
                    model.baseLayerUrl,
                    {
                        //lyrs: "m@177000000",
                        isBaseLayer: true,
...
*/
    ...
    *切换地图
    */
    Ogis.prototype.switchMap = function (baseLayerUrl) {
        // 先移除当前的地图，再添加新的地图
        var map = this.mapModel.map;
        var baselayerid=this.addSuffix("baselayer");
        ...
        this.mapModel.baseLayerUrl = baseLayerUrl;
        var newBaseLayer = this.initMapBaseLayer();

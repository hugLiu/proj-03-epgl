    ...
    self.closeTab = function (data) {
        ...
        if (index == length) {
            ...
            if (index - 1 == 0) {
                ...
                var item = self.getTabByIndex(index - 1);
        ...
        if (currentItem) {
            switch (id) {
                ...
                case "mapType2":
                    ...
                    if (type) {
                        if (index == 1) {
                            getVMapEvent2(currentBoName, type);
                        }
                    }
                    else {
                        joGIS1.ResetContent();
...
var doOtherEvent = function () {
    ...
    gProjName = getCoordinateSystemDefineName();//获取实图的坐标系统
}
...
var getVMapEvent2 = function (currentBoName, type) {
    $.ajax({
...
function clipEvent() {
    ...
    var name = temp.join(',');
    var flag = joGIS1.ClipMap(currentViewModel.currentPointLeft(), currentViewModel.currentPointTop(), currentViewModel.currentPointRight(), currentViewModel.currentPointBottom(), name, 1);
    if (!flag) {
        var cName = getCoordinateSystemDefineName();
        if (gProjName != cName) {
            alert("实图坐标系统是【" + gProjName + "】,虚图坐标系统是【" + cName + "】。\n 坐标系统不一致无法投影坐标。");
        }
    }
}

var getCoordinateSystemDefineName = function () {
    var xml = joGIS1.GetMapInformation();
    var xmlDoc = $.parseXML(xml);
    var type = xmlDoc.getElementsByTagName("Method")[0].getAttribute("type");
    return getProjectionTypeDefinition(type);
}

var getProjectionTypeDefinition = function (type) {
    var name = "";
    switch (type) {
        case "0":
            name = "非地球坐标";
...
var targetSelected = function (e, data) {
    ...
    if (action == "select_node") {
        ...
        currentViewModel.isOriginalMap(true);
...
var getMapType2 = function (name) {
    $.ajax({
        url: "/FlexDraw/GetMapType",
        type: "post",
        data: { workareaName: name },
        success: function (result) {
            currentViewModel.availableMapTypes2(result);
        },
        error: function (message) {
            console.log(message);
        }
...
var getMapLayers = function (isThemeData) {
    ...
    for (var i = 0; i < count; i++) {
        ...
        //是否可选
        entity.unlocked = 0;//一键成图图层默认不可选中

        joGIS1.PM_SetLayerStatus(layerName, entity.visable, entity.unlocked); //设置可见/可选择

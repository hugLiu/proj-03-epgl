...
var SearchViewModel = function () {
    ...
    //搜索总页数
    self.resultTotalPage = ko.observable(1);
    //当前结果页数
    self.resultCurrentPage = ko.observable(1);
    ...
    self.getSearchParamTemp = function (type) {
       ...
       //普通搜索和高级搜索不考虑控件过滤
        ...
        if (type === "simple") {
            ...
            //关键字搜索
            if (self.searchKey() && self.searchKey() != $("#txtKeyword").attr('placeholder')) {
                filterArray.push(
        ...
        if (type === "advance") {
            ...
            if (display=="none") {
                ...
                if (self.selectedMember()) {
                    ...
                    filterArray.push({ "ep.geologyseries": { "$elemMatch": { "value": geologyseries.seqname, "type": "Member" } } });

                } else if (self.selectedFormation()) {
                    //组
                    geologyseries = getGeologySeriesById(self.selectedFormation());
                    filterArray.push({ "ep.geologyseries": { "$elemMatch": { "value": geologyseries.seqname, "type": "Formation" } } });
                } else if (self.selectedSeries()) {
                    //统
                    geologyseries = getGeologySeriesById(self.selectedSeries());
                    filterArray.push({ "ep.geologyseries": { "$elemMatch": { "value": geologyseries.seqname, "type": "Series" } } });
                } else if (self.selectedErathem()) {
                    //界
                    geologyseries = getGeologySeriesById(self.selectedErathem());
                    filterArray.push({ "ep.geologyseries": { "$elemMatch": { "value": geologyseries.seqname, "type": "Erathem" } } });

                } else if (self.selectedSystem()) {
                    //系
                    geologyseries = getGeologySeriesById(self.selectedSystem());
                    filterArray.push({ "ep.geologyseries": { "$elemMatch": { "value":geologyseries.seqname, "type": "System" } } });
    ...
    self.targetSelectDataBind = function (event) {
        ...
        //考虑跨级级联和直接不选父级（应该按照类型过滤）
        var parentids = null;

        var typeflag = false;//当父级都没有选中值时，应该按照类型查询

        global_targets_results = [];

        if (typeid == enumTargetType.Basin) {

            global_targets_results = ["#",null];

        } else if (typeid == enumTargetType.Play) {
            if (self.selectedBasin()) {
                global_targets_results = [self.selectedBasin()];
            } else {
                typeflag = true;
            }
            ...
            if (self.selectedRegion()) {
                global_targets_results = [self.selectedRegion()];
            } else if (self.selectedBasin()) {

                getTargetIdByParentId(self.selectedBasin(), 1);
            } else {
                typeflag = true;
            }
            ...
            if (self.selectedOilField()) {
                global_targets_results = [self.selectedOilField()];

            } else if (self.selectedRegion()) {
                getTargetIdByParentId(self.selectedRegion(), 1);

            } else if (self.selectedBasin()) {
                getTargetIdByParentId(self.selectedBasin(), 2);
            }else {
                typeflag = true;
            }
        }
        if (!typeflag) {
            parentids = global_targets_results;

            if (parentids && parentids.length > 0) {
                queryresult = getTargetByParents(parentids);
            }
        }
        else {
    ...
    self.strataSelectDataBind = function (event) {
        ...
        if (!typeflag) {
            ...
            if (parentids && parentids.length > 0) {
                queryresult = getGeologySeriesByParents(parentids);
            }
        }
        else {
    ...
    //高级搜索目标选项改变
    self.targetSelectChanged = function (event,data) {
        var target$ = $(event.currentTarget);

        var typeid = event.data;
        
        //上级影响下级结果：上级选中值，下级恢复到初始值null
        //下级影响上级结果：下级选中值，上级选中到对应值
        var selectedid = null;
        var node = null;
        ...
        var parentdatas = null;
        var parentid = null;

        if (typeid == enumTargetType.Basin) {
            selectedid = self.selectedBasin();

            if (!selectedid) {
                self.selectedRegion(null);
            } else {
                childid = self.selectedRegion();
                if (childid) {
                    childnode = getTargetByBoId(childid);

                    if (childnode.parent != selectedid) {
                        self.selectedRegion(null);
                    }
                }
            }
            ...
            selectedid = self.selectedRegion();

            if (!selectedid) {
                self.selectedOilField(null);
            } else {
                childid = self.selectedOilField();
                if (childid) {
                    childnode = getTargetByBoId(childid);
                ...
                node = getTargetByBoId(selectedid);
                parentnode = getTargetByBoId(node.parent);

                if (parentid && node.parent != parentid || !parentid) {

                    parentdatas = getTargetsByParent(parentnode.parent);

                    self.basins(parentdatas);
                    self.selectedBasin(parentnode.id);
                    $(".chosen-select[typeid='" + enumTargetType.Basin + "']").change();
                } 
            }

        } else if (typeid == enumTargetType.Field) {
            selectedid = self.selectedOilField();

            if (!selectedid) {
                self.selectedOilReservoir(null);
            } else {
                childid = self.selectedOilReservoir();
                if (childid) {
                    childnode = getTargetByBoId(childid);
                ...
                if (parentid && node.parent != parentid || !parentid) {
                    ...
                    parentdatas = getTargetsByParent(parentnode.parent);
                    self.regions(parentdatas);
                    self.selectedRegion(parentnode.id);
                    $(".chosen-select[typeid='" + enumTargetType.Play + "']").change();
                }
               
            }

        } else if (typeid == enumTargetType.Pool) {
            selectedid = self.selectedOilReservoir();
            ...
            if (selectedid){
                ...
                if (parentid && node.parent != parentid || !parentid) {
                    ...
                    parentdatas = getTargetsByParent(parentnode.parent);
                    self.oilFields(parentdatas);
                    self.selectedOilField(parentnode.id);
                    $(".chosen-select[typeid='" + enumTargetType.Field + "']").change();
                }
            }
        }       
        target$.trigger('chosen:updated');
    ...
    //高级搜索层系选项改变
    self.strataSelectChanged = function (event, data) {
        var target$ = $(event.currentTarget);
        ...
        if (typeid == enumGeologySeriesType.Erathem) {
            selectedid = self.selectedErathem();

            if (!selectedid) {
                self.selectedSystem(null);
            } else {
                childid = self.selectedSystem();
                if (childid) {
                    childnode = getGeologySeriesById(childid);

                    if (childnode.seqpraent != selectedid) {
                        self.selectedSystem(null);
                    }
                }
            }
            ...
            selectedid = self.selectedSystem();

            if (!selectedid) {
                self.selectedSeries(null);
            } else {
                childid = self.selectedSeries();
                if (childid) {
                    childnode = getGeologySeriesById(childid);
                ...
                parentid = self.selectedErathem();
                node = getGeologySeriesById(selectedid);
                parentnode = getGeologySeriesById(node.seqpraent);

                if (parentid && node.seqpraent != parentid || !parentid) {

                    parentdatas = getGeologySeriesByParent(parentnode.seqpraent);

                    self.erathem(parentdatas);
                    self.selectedErathem(parentnode.seqid);
                    $(".chosen-select[typeid='" + enumGeologySeriesType.Erathem + "']").change();
                }
            }

        } else if (typeid == enumGeologySeriesType.Series) {
            selectedid = self.selectedSeries();

            if (!selectedid) {
                self.selectedFormation(null);
            } else {
                childid = self.selectedFormation();
                if (childid) {
                    childnode = getGeologySeriesById(childid);
                ...
                if (parentid && node.seqpraent != parentid || !parentid) {
                    ...
                    parentdatas = getGeologySeriesByParent(parentnode.seqpraent);
                    self.system(parentdatas);
                    self.selectedSystem(parentnode.seqid);
                    $(".chosen-select[typeid='" + enumGeologySeriesType.System + "']").change();
                }

            }

        } else if (typeid == enumGeologySeriesType.Formation) {
            selectedid = self.selectedFormation();

            if (!selectedid) {
                self.selectedMember(null);
            } else {
                parentid = self.selectedSeries();
                node = getGeologySeriesById(selectedid);
                parentnode = getGeologySeriesById(node.seqpraent);
                ...
                if (parentid && node.seqpraent != parentid || !parentid) {
                    ...
                    parentdatas = getGeologySeriesByParent(parentnode.seqpraent);
                    self.series(parentdatas);
                    self.selectedSeries(parentnode.seqid);
                    $(".chosen-select[typeid='" + enumGeologySeriesType.Series + "']").change();
                }
            }
        } else if (typeid == enumGeologySeriesType.Member) {
            selectedid = self.selectedMember();

            if (selectedid) {
                parentid = self.selectedFormation();
                node = getGeologySeriesById(selectedid);
                parentnode = getGeologySeriesById(node.seqpraent);
                ...
                if (parentid && node.seqpraent != parentid || !parentid) {
                    ...
                    parentdatas = getGeologySeriesByParent(parentnode.seqpraent);
                    self.formation(parentdatas);
                    self.selectedFormation(parentnode.seqid);
                    $(".chosen-select[typeid='" + enumGeologySeriesType.Formation + "']").change();
                }
            }
        }
        target$.trigger('chosen:updated');
    ...
    self.searchResult = function (type) {
        ...
        if (type === "loadmore") {
            ...
            if (type === "simple") {
                ...
                if (key && key != "" && key != $("#txtKeyword").attr('placeholder')) {
                    var param = {
                        "userid": userid,
                        "smode": 2,
                        "keyword": key
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                    var date = toDateTime(item.indexeddate, "yyyy-MM-dd hh:mm:ss", new Date()).Format("yyyy-MM-dd hh:mm:ss");
                    var temp = {
                        iiid: item.iiid,
                ...
                if (pageIsInit) {
                    ...
                    self.resultTotalPage(pageCount);//更新结果总页码数
                    self.resultCurrentPage(1);//初始还原当前页数
    ...
    self.renderPageNav = function (totalCount) {
        var options = {
            ...
            onPageChanged: function (e, oldPage, newPage) {
                self.resultCurrentPage(newPage);
...
//根据parent查找该parent下的所有子目标
var getTargetsByParent=function(parentid){
    var queryresult = Enumerable.From(global_targets)
...
//根据parentids查找bo
var getTargetByParents = function (parentids) {
    var queryresult = Enumerable.From(global_targets)
...
//根据parent查找该parent下的所有子层系
var getGeologySeriesByParent = function (parentid) {
    var queryresult = Enumerable.From(global_geologyseries)
        .Where(function (i) { return i.seqpraent == parentid; }).ToArray();
    return queryresult;
};
//根据parentids查找层系
var getGeologySeriesByParents = function (parentids) {
    var queryresult = Enumerable.From(global_geologyseries)

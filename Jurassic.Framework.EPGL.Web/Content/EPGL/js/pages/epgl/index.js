        ...
        success: function (result) {
            ...
            if (isTable) {
                ...
                for (var i = 0; i < data.length; i++) {
                    var item = data[i];
                    //var date = toDateTime(item.indexeddate, "yyyy-MM-dd hh:mm:ss", new Date()).Format("yy/MM/dd");
                    //var date = dateFormat(strdate, "yyyy-MM-dd hh:mm:ss");
                    var temp = {
                        index: i,
                        iiid: item.iiid,
...
var ViewModel = function () {
    ...
    self.dateFormat = function (value) {
        ...
        if (value.indexOf("/Date(") >= 0)
            date = toJsonDate(value);
        else
            date = toDateTime(value, "yyyy-MM-dd hh:mm:ss", new Date());

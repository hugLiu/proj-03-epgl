        ...
        success: function (result) {
            if (result) { 
                for (var i = 0; i < result.length; i++) {
                    ...
                    config.style.layer[key].text.fillColor = textcolor.split(',');
                } 
            } 
        }
    });
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result) {
                        ...
                        ogis.addVecLayerByGml(layerid, layername, true, false, true, result);
                        global_layersdata.push({
                            id: layerid,
                            text: layername,
                            layId: ogis.addSuffix(layerid),
                            layName: layername,
                            visable: 1,
                            unlocked: 1
                        });
...
var mapEventBind = function () {
    ...
    impfile.fileinput({
        ...
        uploadUrl: '/EPGL/AddProject?id=' + fileId,

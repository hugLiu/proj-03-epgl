...
var GeoMapViewModel = function () {
    ...
    self.searchResultClick = function (item) {
        if (item.BOTNAME != enumBoType.Well) {
            var find = JoGis4.FindEleByName(item.BONAME, 3);
...
var configGeoMap = function () {
    ...
    //初始化工具栏位置
    $("#geoMapPane #tool_pane").css("top", "100%").css("top", "-=268px");
    $("#geoMapPane #tool_pane").css("left", "100%").css("left", "-=40px");
};
        ...
        success: function (retrieveData) {
            ...
            if (retrieveData && retrieveData.length > 0) {
                ...
                if (ret == "jogis001") {
                    ...
                    //1.更新图层列表
                    geomap_layersdata = getMapLayers();
                    loadGeoLayers(geomap_layersdata);

...
var mapInit = function (maptype) {
    ...
    //初始化工具栏位置
    $("#panelMap #tool_div").css("top", "100%").css("top", "-=268px");
    $("#panelMap #tool_div").css("left", "100%").css("left", "-=40px");
};
...
//切换地图
var switchMap = function (maptype) {
    if (maptype === "terrain") {
        outputPath = gloabal_map_url + "/terrain/";
    } else {
        outputPath = gloabal_map_url + "/satellite/";
    }
    ogis.switchMap(outputPath + "{z}/{y}/{x}" + format);
}

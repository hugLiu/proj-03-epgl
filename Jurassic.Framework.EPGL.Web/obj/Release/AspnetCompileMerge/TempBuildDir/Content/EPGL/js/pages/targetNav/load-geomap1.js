...
var GeoMapViewModel = function () {
    ...
    self.searchResultClick = function (item) {
        if (item.BOTNAME != enumBoType.Well) {
            var find = JoGis4.FindEleByName(item.BONAME);
        ...
        //var node = targetTree.get_node(item.BOID).original;
        //var parentNode = null;
        ...
        //        parentNode = targetTree.get_node(node.parent).original;
        //        break;
        //    case enumBoType.Field:
        //        parentNode = targetTree.get_node(node.parent);
        ...
        //        var properties = item.BOPropertySet;
        //        if (properties) {
        //            for (var i = 0; i < properties.length; i++) {
        //                if (properties[i].Key == "井所属工区") {
        //                    parentNode = getCurTargetNode(properties[i].Value);
        //                    break;
        //                }
        //            }
        //        }
        ...
        //    loadNavMap(parentNode.typeid, parentNode.aliasnames, item.BONAME);
        //}


        //loadSearchNavMap(item.BOID, item.BONAME, item.BOTNAME, item.MapDataUrl, item.MapItemId, item.MapBos);
        //折叠面板折叠
        $("#historyCollapse").collapse("hide");
    }
    ...
    //当前图元名称(是目标时该图元的名称需与目标名称保持一致）
    self.currentEleName = ko.observable();
    ...
    self.navigateTo = function (data) {
        ...
        //    loadNavMap(currentTargetNode.typeid, currentTargetNode.aliasnames);
        //    //更改目标树数据
        //    loadTargetTree();
        //}
        ...
        success: function (retrieveData) {
            ...
            if (retrieveData && retrieveData.length > 0) {
                ...
                if (ret == "jogis001") {
                    ...
                    //2.如果需要选中图元，则选中
                    if (name) {
                        var find = JoGis4.FindEleByName(name);
                        if (find&&!$.isEmptyObject(find)) {
                            var status = joGIS1.PM_SetLayerStatus(find.LayerName, 1, 1);
                            selFindElement(find.ElementType, find.ElementID);
                        }                        
...
var loadNavMap = function (boType, aliasNames, name) {
    ...
    //当不需要选择图元的时候，以当前选中节点为当前图元图元名称，加载相关图件
    //if (!name) {        
        
    //}
    //currentViewModel.currentEleName(currentTargetNode.text);
    //loadRelevantMap(aliasNames);
...
//根据搜索目标获取搜索目标所在导航图(向上导航)
var loadSearchNavMap = function (boId, boName, botName, mapDataUrl, mapItemId, mapBos) {
    //1.判断当前导航级别，如果目标所在导航图与当前导航图匹配则不需要重新加载图件
    var navLevel = getNavLevelAndMapTypeByBoType(botName).navLevel;
    if (navLevel != currentViewModel.currentNavLevel()) {
        //2.加载当前图元所在地图,并选中图元

        if (botName != enumBoType.Basin) {
            loadGeoMapFile(mapDataUrl, mapItemId);
        } else {
            loadGeoMapFile(mapDataUrl);
        }
    ...
    //5.选中当前图元对应的目标节点,井位选中所在油藏节点
    if (botName == enumBoType.Well) {
        //todo 获取油藏节点
        if (mapBos) {
            var parentBoName = null;
            var item = null;
            for (var i = 0; i < mapBos.length; i++) {
                item = mapBos[i];
                if (item.TYPE == enumBoType.Pool) {
                    parentBoName = bo.VALUE;
                    break;
                }
            }
            if (parentBoName) {
                var node = getCurTargetNode(parentBoName);
                targetTree.deselect_all(true);
        ...
        targetTree.deselect_all(true);
        targetTree.select_node(boId);
    }
    //6.更新图元名称
    currentViewModel.currentEleName(boName);
};
var renderTargetTree = function (jsonData) {
    if (targetTree) {
        $("#targets_tree").jstree("destroy");
    }
    ...
    $("#targets_tree").jstree({
        ...
        if (currentTargetNode) {
            ...
            if (currentTargetNode.typeid == enumBoType.Play) {
                node = getParTargetNode(currentTargetNode.parent);
            }
            var eleName = null;
            if (currentMapElement) {
                eleName = currentMapElement.elementName || currentMapElement;
            }
            loadNavMap(node.typeid, node.aliasnames, eleName);
        }else {
            node = jsonData[0];

            targetTree.deselect_all(true);
            targetTree.select_node(node);
            loadNavMap(node.typeid, node.aliasnames);
        }
...
//树节点选择事件
var targetSelected = function (event, data) {
    ...
    if (action == "select_node") {
        ...
        currentViewModel.currentEleName(currentTargetNode.text);
        ////3.获取相关图件
        loadRelevantMap(currentTargetNode.aliasnames, currentTargetNode.typeid);
        ...
        if (data.event && data.event.type == "click") {
            ...
            //如果是区带，需要选中图元（当前导航）
            //if (boType == enumBoType.Play) {
            //    name = currentTargetNode.text;

            //    if (navLevel == currentNavLevel) {
            //        var find = JoGis4.FindElementByNames(name);
            //        var status = joGIS1.PM_SetLayerStatus(find.LayerName, 1, 1);//更新图层状态
            //        selFindElement(find.ElementType, find.ElementID, name);
            //    } else {
            //        var parentNode = targetTree.get_node(currentTargetNode.parent).original;
            //        if (parentNode) {
            //            boType = parentNode.typeid;
            //            aliasNames = parentNode.aliasnames;
            //        }
            //        loadNavMap(boType, aliasNames, name);
            //    }
            //} else {
            //    loadNavMap(boType, aliasNames);
            //}
        }
        currentViewModel.toolTipVisible(false);
    }
...
var eventBind = function () {
    ...
    $("#btn_reset,#tool_reset").click(function () {
    ...
    function joGIS1::SelectedElement(nCount, nKeyType) {
        ...
        for (i = 0; i < nCount; i++) {
            ...
            //1.更改图元名称，默认显示无相关图件
            currentViewModel.currentEleName(name);
            showRelevantMap();
            ...
            if (nType != 3 && nType != 11) {
                ...
                if (nType == 3) {
                    ...
                    if (!node) {
                        ...
                        //if (navLevel == 1) {
                        //    //5.盆地级显示图元介绍信息（缩略图和简介文字）

                        //    //判断当前面积图元是否可导航（区带不能导航，油田可以）
                        //    var isCanNav = false;
                        //    if (node.level == 3) {
                        //        isCanNav = true;
                        //    }
...
var deSelFindElement = function () {
    ...
    if (parentNode) {
        ...
        targetTree.select_node(parentNode);
        //更新图元名称，加载相关图件
        currentViewModel.currentEleName(parentNode.text);

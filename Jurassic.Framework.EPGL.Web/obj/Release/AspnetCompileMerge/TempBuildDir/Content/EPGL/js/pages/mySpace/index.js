...
var MySpaceViewModel = function () {
    ...
    //加载我的自定义图件
    self.loadCustomList = function () {
        //var data = Enumerable.From(dataGdbThumbs)
        //    .Where(function(i) { return i.typeid == "hyqmjt"; }).ToArray();
        ////.ForEach((item) => {
        ////    item["url"] = "/DemoData/thumb/" + item.typename + "/" + item.src;
        ////    item["thumb"] = "/DemoData/thumb-sm/" + item.typename + "/" + item.thumb;
        ////});
        //for (var j = 0; j < data.length; j++) {
        //    data[j]["src"] = "/DemoData/thumb/" + data[j].typename + "/" + data[j].src;
        //    data[j]["thumbSrc"] = "/DemoData/thumb-sm/" + data[j].typename + "/" + data[j].thumb;
        //}

        //currentModel.myCustomList(data);
    };
